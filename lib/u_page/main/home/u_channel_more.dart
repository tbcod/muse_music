    ...
    if (controller.info["type"] == "video") {
      ...
      return GestureDetector(
        onTap: () {
          AppLog.e(childItem);
          EventUtils.instance
              .addEvent("det_playlist_show", data: {"from": "artist_playlist"});
...
class UserChannelMoreController extends GetxController with StateMixin {
  ...
  Future bindMoreData() async {
    ...
    var result = await ApiMain.instance.getYoutubeData(info["moreBrowseId"],
        params: info["moreParams"], nextData: {"continuation": moreToken});
    ...
    if (info["type"] == "video") {
      ...
      for (Map oldItem in oldList) {
        try {
          var videoId = oldItem["richItemRenderer"]["content"]["videoRenderer"]
                  ["videoId"] ??
              "";
          var title = oldItem["richItemRenderer"]["content"]["videoRenderer"]
                  ["title"]["runs"][0]["text"] ??
              "";
          var cover = oldItem["richItemRenderer"]["content"]["videoRenderer"]
      ...
      for (Map oldItem in oldList) {
        try {
          var playlistId = oldItem["richItemRenderer"]["content"]
                  ["playlistRenderer"]["playlistId"] ??
              "";
          var title = oldItem["richItemRenderer"]["content"]["playlistRenderer"]
                  ["title"]["simpleText"] ??
              "";
          var cover = oldItem["richItemRenderer"]["content"]["playlistRenderer"]
