...
class ApiMain extends BaseApi {
  ...
  Future<BaseModel> getData(String browseId,
      {String? params, Map? nextData, String? videoId}) async {
    ...
    var result = await httpRequest(url,
        method: HttpMethod.post, contentType: "application/json", body: body);
  ...
  Future<BaseModel> getVideoInfo(String videoId,
      {bool toastBlack = true}) async {
    ...
    BaseModel result = await httpRequest(url,
        method: HttpMethod.post, contentType: "application/json", body: body);
  ...
  Future<BaseModel> getSearchResult(String input,
      {String params = "", Map? nextData}) {
  ...
  Future<BaseModel> getVideoNext(String videoId,
      {bool isMoreVideo = false, String continuation = ""}) {
  ...
  Future<BaseModel> getYoutubeData(String browseId,
      {String? params, Map? nextData, String? videoId}) async {
    var nowTime = DateTime.now();
    String date =
        "${nowTime.year}${nowTime.month.toString().padLeft(2, "0")}${nowTime.day.toString().padLeft(2, "0")}";
  ...
  Future<BaseModel> getYoutubeNext(String videoId,
